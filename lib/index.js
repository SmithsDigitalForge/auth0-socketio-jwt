...
const socketAuthTimeouts = {};

function getBaseSocketId(id) {
...
function noQsMethod(options) {
  ...
  return function (socket) {
    ...
    console.log(`Inside top. id: ${socket.id} , namespace: ${socket.nsp.name}`);

    if(options.required){
      // When connecting a socket with a namespace (and multiplex = true), there is an initial connection request with
      // the base socket id, followed by another connection with the namespace#socket.id, followed by an authenticate
      // request with the namespace#socket.id.
      // Without the custom code below the namespaced sockets get disconnected because two connections are made,
      // creating two timeout callbacks; but only one of them is cleared in the 'authenticate' handler below.

      if (socket.nsp.name !== '' && socket.nsp.name !== '/') {
        var baseId = getBaseSocketId(socket.id);

        if (socketAuthTimeouts[baseId]) {
          clearTimeout(socketAuthTimeouts[baseId]);
          delete socketAuthTimeouts[baseId];
        }

        socketAuthTimeouts[socket.id] = setTimeout(function () {
    ...
    socket.on('authenticate', function (data) {
      if(options.required){
        ...
        if (socketAuthTimeouts[socket.id]) {
          clearTimeout(socketAuthTimeouts[socket.id]);
          delete socketAuthTimeouts[socket.id];
        }
      }
      // error handler
